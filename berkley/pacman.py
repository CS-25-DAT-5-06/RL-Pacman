...
"""
from berkley.game import GameStateData
from berkley.game import Game
from berkley.game import Directions
from berkley.game import Actions
from berkley.util import nearestPoint
from berkley.util import manhattanDistance
import berkley.util as util
import berkley.layout as layout
import sys
import types
import time
import random
...
def moduleLoadAgent(pacman, nographics):
    ...
    for modulename in agentModules:  
        try:
            module = importlib.import_module(f"berkley.{modulename.split('.')[0]}")
        except ImportError:
            print("here!")
            continue
        if pacman in dir(module):
